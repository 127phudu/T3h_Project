...
@section('content')
    ...
    @endif
    <form  class="form-horizontal" action="{{ url('admin/banners/'.$banner->id) }}" method="post">
        @csrf
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <input type="text" name="link" class="form-control" id="link" value="{{ $banner->link }}" required>
            </div>
            ...
            <div class="col-sm-10">
                <select name="location_id">
                    ...
                    @foreach($locations as $key_location => $location)
                        <option value="{{ $key_location }}" {{ ($banner->location_id == $key_location) ? 'selected' : '' }}>{{ $location }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                ...
                <span class="input-group-btn">
                    ...
                    <input id="thumbnail1" class="form-control" type="text" name="image" value="{{ $banner->image }}" placeholder="Chọn ảnh">
                </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <textarea type="text" name="intro" class="form-control tinymce" id="intro"  required> {{ $banner->intro }} </textarea>
            </div>
        </div>
        <div class="form-group">
            <label for="desc" class="control-label col-sm-2">Mô tả: </label>
            <div class="col-sm-10">
                <textarea type="text" name="desc" class="form-control tinymce" id="desc"  required> {{ $banner->desc }} </textarea>
            </div>
        </div>
