...
@section('content-header')
    <h1>Sửa danh mục bài viết: {{ $cat->name }}</h1>
@endsection
...
@section('content')
    ...
    @endif
    <form  class="form-horizontal" action="{{ url('admin/content/category/'.$cat->id) }}" method="post">
        @csrf
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <textarea type="text" name="intro" class="form-control tinymce" id="intro"  required> {{ $cat->intro }} </textarea>
            </div>
        </div>
        <div class="form-group">
            <label for="desc" class="control-label col-sm-2">Mô tả: </label>
            <div class="col-sm-10">
                <textarea type="text" name="desc" class="form-control tinymce" id="desc"  required> {{ $cat->desc }} </textarea>
            </div>
        </div>
